...
static int _udp_keep_alive_changed = 0;
...
#ifndef MINISIZE
int
eXosip_find_free_port (int free_port, int transport)
{
  int res1;
  int res2;
  ...
  for (count = 0; count < 8; count++)
    {
      ...
      if (res1 != 0)
        return res1;
      res2 =
        eXosip_get_addrinfo (&addrinfo_rtcp, "0.0.0.0", free_port + count * 2 + 1,
                             transport);
      if (res2 != 0)
        {
...
_eXosip_keep_alive (void)
{
  ...
  if ((mtimer.tv_sec == 0 && mtimer.tv_usec == 0) || _udp_keep_alive_changed == 1)
    {
      /* first init */
      osip_gettimeofday (&mtimer, NULL);
      add_gettimeofday (&mtimer, eXosip.keep_alive);
	  _udp_keep_alive_changed = 0;
    }
