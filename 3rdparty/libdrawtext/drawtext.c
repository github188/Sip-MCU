...
#ifdef CHINESE
  FT_Glyph   glyphs[ 65535 ];
  FT_Bitmap  bitmaps[ 65535 ];
  int        advance[ 65535 ];
  int        bitmap_left[ 65535 ];
  int        bitmap_top[ 65535 ];
  unsigned int glyphs_index[ 65535 ];
#else
  FT_Glyph   glyphs[ 255 ];
    ...
    *ctxp = av_mallocz(sizeof(ContextInfo));
    ...
    while ((c = getopt(argc, argv, "f:t:T:x:y:s:c:C:bo")) > 0) {
      ...
      case 'f':
        ...
        break;
      case 't':
        ci->text = (unsigned char *)av_strdup(optarg);
        break;
      case 'T':
	...
	for (c=0; c < 255; c++)
...

      {
        /* Load char */
#ifdef CHINESE
		error = FT_Load_Char( ci->face, c, FT_LOAD_RENDER | FT_LOAD_MONOCHROME );
#else 
        ...
        /* Save glyph index */
#ifdef CHINESE
		ci->glyphs_index[c] = FT_Get_Char_Index( ci->face, c );
#else
...
void Process(void *ctx, void *data,int width, int height, int64_t pts)
{
  ...
  if (ci->file)
    {
      ...
      if (fd < 0)
        ...
        {
          int l = read(tbuff, sizeof(tbuff) - 1,1,fd);
  ...
  strftime((char *)buff, sizeof(buff), (const char *)text, localtime(&now));

  text = buff;

#ifndef CHINESE
  size = strlen((const char *)text);
//change the code for be fitting for the chinese displaying 
#else 
	setlocale(LC_ALL,"");
//if it is necessary ,to use malloc  function for the wText
	wchar_t wText[50]={L'0'};	
	mbstowcs(wText,(const char *)text,40);
	size=wcslen(wText);
#endif
