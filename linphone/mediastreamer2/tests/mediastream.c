...
static void run_media_streams(int localport, const char *remote_ip, int remoteport, int payload, const char *fmtp,
          int jitter, int bitrate, MSVideoSize vs, bool_t ec, bool_t agc, bool_t eq, const char* local_port, bool_t enabled_udt);
...
int main(int argc, char * argv[])
{
	...
	rtp_profile_set_payload(&av_profile,114,&payload_type_speex_uwb);
	rtp_profile_set_payload(&av_profile,112,&payload_type_ilbc);
	rtp_profile_set_payload(&av_profile,113,&payload_type_amr);
	rtp_profile_set_payload(&av_profile,106,&payload_type_bv16);
	rtp_profile_set_payload(&av_profile,107,&payload_type_bv32);
#ifdef VIDEO_ENABLED
	...
	vs.width=MS_VIDEO_SIZE_VGA_W;
	vs.height=MS_VIDEO_SIZE_VGA_H;
	if (argc<4) {
		printf("%s",usage);
		return -1;
	}
	...
	run_media_streams(localport,ip,remoteport,payload,fmtp,jitter,bitrate,vs,ec,agc,eq,localportstr,use_udt);
	return 0;
}
...
//	const char *remoteport_str;
	...
	if (pt->type!=PAYLOAD_VIDEO){
		...
		video_stream_start(video,profile,
					...
					payload,
					jitter,
					ms_web_cam_manager_get_cam(ms_web_cam_manager_get(),"Built-in iSight"));
