...
//	if(call->state == LinphoneCallPaused) return;
...
static void auth_requested(SalOp *h, const char *realm, const char *username){
	...
	if (ai && (ai->works || ai->usecount<3)){
		...
		sai.realm=(ai->realm!=NULL)?ai->realm:realm;
...
static void register_failure(SalOp *op, SalError error, SalReason reason, const char *details,int code){
	LinphoneCore *lc=(LinphoneCore *)sal_get_user_pointer(sal_op_get_sal(op));
	LinphoneProxyConfig *cfg=(LinphoneProxyConfig*)sal_op_get_user_pointer(op);
	...
	linphone_proxy_config_set_state(cfg,LinphoneRegistrationFailed,details,reason);
}
