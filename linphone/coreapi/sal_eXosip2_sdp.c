...
static void add_line(sdp_message_t *msg, int lineno, const SalStreamDescription *desc){
	...
	/*添加 ICE 收集信息*/
	if(desc->type == SalAudio){ //音频端口
	    int i;
		//sdp_message_a_attribute_add (msg, lineno,
		//	osip_strdup ("rtcp"), osip_strdup("5000"));

		for(i=0;i<SAL_ENDPOINT_CANDIDATE_MAX;i++)
		{
			if(desc->candidates[i].addr[0]!='\0')
			{
				ms_message("add ice candidate:%d t UDP 1.0 %s %d ",i,desc->candidates[i].addr,desc->candidates[i].port);
			}
		}
