...
static bool_t registration_failure(Sal *sal, eXosip_event_t *ev){
	...
	switch(status_code){
		...
		default:
			...
			sal->callbacks.register_failure(op,se,sr,reason,status_code);
	}
	return TRUE;
}
...
int sal_call_send_picture_fast_update(SalOp *h){
	osip_message_t *msg=NULL;
	char info_body[256];
	char clen[10];
	static char pfu_msg[] = "<?xml version=\"1.0\" encoding=\"utf-8\" ?>\n"
		" <media_control>\n"
		"  <vc_primitive>\n"
		"   <to_encoder>\n"
		"    <picture_fast_update/>\n"
	...
	eXosip_call_build_info(h->did,&msg);
	if (msg){
		snprintf(info_body, sizeof(info_body), "%s",pfu_msg);
