...
//extern void g729_plugin_init();
extern void conf_itc_init(void);
...
static PayloadType * lookup_payload_by_mime(RtpProfile *prof, const char *mime_name){
	PayloadType *candidate=NULL;
	int i;
	PayloadType *it;
	for(i=0;i<127;++i){
		it=rtp_profile_get_payload(prof,i);
		if (it!=NULL && strcasecmp(mime_name,it->mime_type)==0){
			if(ms_filter_codec_supported(mime_name))
				return it;
		}
	}
	return candidate;
...
static void disable_all_payload(const MSList *codecs){

	const MSList *elem = codecs;
	for (;elem!=NULL;elem=elem->next){
		payload_type_set_enable((PayloadType *)elem->data,0);
	}
	return NULL;
}
...
static void sip_config_read(LinphoneCore *lc)
{
	...
	tmp=lp_config_get_int(lc->config,"sip","inc_timeout",25);
...
#ifdef VIDEO_ENABLED
	libmsx264_init();
//	linphone_core_assign_payload_type(&payload_type_jpeg,26,NULL);
  ...
  linphone_core_assign_payload_type(&payload_type_theora,97,NULL);
	...
	linphone_core_assign_payload_type(&payload_type_mp4v,114,"profile-level-id=3");
//  linphone_core_assign_payload_type(&payload_type_x_snow,100,NULL);
//	linphone_core_assign_payload_type(&payload_type_h264,109, NULL); /*; profile-level-id=42800d; max-mbps=40000; max-fs=1792; max-br=1600*/
	linphone_core_assign_payload_type(&payload_type_h264,110,"packetization-mode=1");
#endif
...
int linphone_core_set_audio_codecs_by_string(LinphoneCore *lc, char *list)
{
	MSList *new_list = NULL;
	MSList *old_list = lc->codecs_conf.audio_codecs;
	PayloadType *pt;
	char *tok = NULL;
	...
	disable_all_payload(old_list);

	while(tok)
	{
		pt = lookup_payload_by_mime(&av_profile,tok);
		if(pt)
		{
			payload_type_set_enable(pt,1);
			new_list = ms_list_append(new_list,(void *)pt);
			old_list = ms_list_remove(old_list,pt);
		}
	...
	for (;old_list!=NULL;old_list=old_list->next){
		payload_type_set_enable((PayloadType *)old_list->data,0);
		new_list = ms_list_append(new_list,(void *)old_list->data);
	}

	ms_list_free(old_list);
	lc->codecs_conf.audio_codecs = NULL;
	linphone_core_set_audio_codecs(lc,new_list);
	return 0;
}
...
int linphone_core_set_video_codecs_by_string(LinphoneCore *lc, char *list)
{
	MSList *new_list = NULL;
	MSList *old_list = lc->codecs_conf.video_codecs;
	PayloadType *pt;
	char *tok = NULL;
	...
	ms_list_free(old_list);
	lc->codecs_conf.video_codecs = NULL;
	linphone_core_set_video_codecs(lc,new_list);
	return 0;
}
...
void linphone_core_set_sip_port(LinphoneCore *lc,int port)
{
	LCSipTransports tr;
	memset(&tr,0,sizeof(tr));
	tr.udp_port=linphone_core_get_idel_udp_port(port);
	linphone_core_set_sip_transports (lc,&tr);
}
...
void linphone_core_iterate(LinphoneCore *lc){
	...
	while(calls!= NULL){
		...
		/*if ((call->dir==LinphoneCallIncoming && call->state!=LinphoneCallStreamsRunning && call->state != LinphoneCallPaused) 
			|| call->state==LinphoneCallOutgoingRinging 
			|| call->state != LinphoneCallPaused
			|| call->state != LinphoneCallResuming
...
int linphone_core_accept_call(LinphoneCore *lc, LinphoneCall *call)
{
	...
	/*can accept a new call only if others are on hold */
	{
		...
		for(elem=lc->calls;elem!=NULL;elem=elem->next){
			...
			if (c!=call && (c->state!=LinphoneCallPaused && c->state!=LinphoneCallPausing) && call->state==LinphoneCallInConferencing){
...
#ifdef ENABLED_MCU_MEDIA_SERVER
	if(call->state == LinphoneCallInConferencing && call->user_pointer!=NULL){
		LinphoneConference *conf = (LinphoneConference *)call->user_pointer;

		if(linphone_core_call_in_conferencing(lc,conf,call)){
			linphone_conference_remove_mb(lc,conf,linphone_conference_find_mb_by_call(lc,conf,call));
		}
	}
#endif // ENABLED_MCU_MEDIA_SERVER
...
int linphone_core_get_play_level(LinphoneCore *lc)
{
	...
	if(sndcard) lc->sound_conf.play_lev =  ms_snd_card_get_level(sndcard,MS_SND_CARD_PLAYBACK);
...
{
	MSSndCard *sndcard=lc->sound_conf.ring_sndcard;
	if(sndcard) lc->sound_conf.ring_lev =  ms_snd_card_get_level(sndcard,MS_SND_CARD_PLAYBACK);
...
int linphone_core_get_rec_level(LinphoneCore *lc){
	...
	if(sndcard) lc->sound_conf.rec_lev =  ms_snd_card_get_level(sndcard,MS_SND_CARD_MASTER);
...
extern int get_speech_volume_level(int *speech_input_level, int *speech_output_level);

int linphone_get_mic_speech_level(LinphoneCore *lc)
{	
	int level_in,level_out;
	get_speech_volume_level(&level_in, &level_out);
	return level_in;
}

int linphone_get_spk_speech_level(LinphoneCore *lc)
{
	int level_in,level_out;
	get_speech_volume_level(&level_in, &level_out);
	return level_out;
...
MSSndCard *linphone_core_sound_get_default_capture_device(LinphoneCore *lc){
	return lc->sound_conf.capt_sndcard;
}

MSSndCard *linphone_core_sound_get_default_playback_device(LinphoneCore *lc){
...
int  linphone_core_get_sound_devices_count(LinphoneCore *lc)
{
	int ndev=0;
	const MSList *elem=ms_snd_card_manager_get_list(ms_snd_card_manager_get());
	ndev=ms_list_size(elem);
...
int linphone_core_get_video_devices_count(const LinphoneCore *lc)
{
	int ndev;
	const MSList *elem;
	/* retrieve all video devices */
...
void linphone_core_enable_ns(LinphoneCore *lc, bool_t val){
	lc->sound_conf.ns=val;
}

bool_t linphone_core_ns_enabled(const LinphoneCore *lc){
	return lc->sound_conf.ns;
}
/**
...
void linphone_core_send_picture_fast_update(LinphoneCall *call)
{
	if (call==NULL){
		ms_warning("send_picture_fast_update(): no active call");
		return;
	}
	ms_message("sent: <application/media_control+xml> picture_fast_update !");
	sal_call_send_picture_fast_update(call->op);	
}
...
void linphone_core_play_video_file(LinphoneCore *lc, const char *file){
	LinphoneCall *call=linphone_core_get_current_call(lc);
	if (file!=NULL) {
		if (call && call->videostream)
			video_stream_play_file(call->videostream,file);
	}
}

void linphone_core_play_video_file_stop(LinphoneCore *lc){
	LinphoneCall *call=linphone_core_get_current_call(lc);
		if (call && call->videostream)
			video_stream_play_file_stop(call->videostream);
}
...
{
	lp_config_set_string(lc->config,"video","device",linphone_core_get_video_device(lc));
...
#ifdef ENABLED_MCU_MEDIA_SERVER
	p->conf_mode=linphone_core_get_mcu_mode(lc);
#endif // ENABLED_MCU_MEDIA_SERVER
	return p;
}
