...
static int recvStunResponse(ortp_socket_t sock, char *ipaddr, int *port, int *id){
	...
	if (len>0){
		struct in_addr ia;
		if(!stunParseMessage(buf,len, &resp)) return -1;
		*id=resp.msgHdr.tr_id.octet[0];
		...
		}else if (resp.hasXorMappedAddress){
...
void linphone_core_run_stun_tests(LinphoneCore *lc, LinphoneCall *call){
	...
	if (server!=NULL){
		...
		socklen_t ss_len;
		ortp_socket_t audio_sock=-1,audio_rtcp_sock=-1, video_sock=-1,video_rtcp_sock=-1;
		bool_t video_enabled=linphone_core_video_enabled(lc);
		bool_t got_audio,got_video,got_audio_rtcp,got_video_rtcp;
		bool_t cone_audio=FALSE,cone_video=FALSE;
		struct timeval init,cur;
		SalEndpointCandidate *ac,*vc, *ac_rtcp,*vc_rtcp;
		
		ac=&call->localdesc->streams[0].candidates[0];
		vc=&call->localdesc->streams[1].candidates[0];
		ac_rtcp=&call->localdesc->streams[0].candidates[1];
		vc_rtcp=&call->localdesc->streams[1].candidates[1];
		...
		audio_sock=create_socket(call->audio_port);
		if (audio_sock<0) return;

		audio_rtcp_sock=create_socket(call->audio_port+1);
		if (audio_rtcp_sock<0) return;

		if (video_enabled){
			video_sock=create_socket(call->video_port);
			if (video_sock<0) return ;

			video_rtcp_sock=create_socket(call->video_port+1);
			if (video_rtcp_sock<0) return;
		}

		sendStunRequest(audio_sock,(struct sockaddr*)&ss,ss_len,11,TRUE);
		sendStunRequest(audio_sock,(struct sockaddr*)&ss,ss_len,1,FALSE);

		sendStunRequest(audio_rtcp_sock,(struct sockaddr*)&ss,ss_len,33,TRUE);
		sendStunRequest(audio_rtcp_sock,(struct sockaddr*)&ss,ss_len,3,FALSE);

		if (video_sock>=0){
			sendStunRequest(video_sock,(struct sockaddr*)&ss,ss_len,22,TRUE);
			sendStunRequest(video_sock,(struct sockaddr*)&ss,ss_len,2,FALSE);
		}
		
		if (video_rtcp_sock>=0){
			sendStunRequest(video_rtcp_sock,(struct sockaddr*)&ss,ss_len,44,TRUE);
			sendStunRequest(video_rtcp_sock,(struct sockaddr*)&ss,ss_len,4,FALSE);
		}

		got_audio=FALSE;
		got_video=FALSE;
		got_audio_rtcp=FALSE;
		got_video_rtcp=FALSE;
		...
		do{
			...
			if (recvStunResponse(audio_sock,ac->addr,
						&ac->port,&id)>0){
			...
			if (recvStunResponse(audio_rtcp_sock,ac_rtcp->addr,
				&ac_rtcp->port,&id)>0){
					ms_message("STUN test result: local audio rtcp port maps to %s:%i",
						ac_rtcp->addr,
						ac_rtcp->port);
			...
			if (recvStunResponse(video_sock,vc->addr,
							&vc->port,&id)>0){
			...
			if (recvStunResponse(video_rtcp_sock,vc_rtcp->addr,
				&vc_rtcp->port,&id)>0){
					ms_message("STUN test result: local video rtcp port maps to %s:%i",
						vc_rtcp->addr,
						vc_rtcp->port);
		...
		}while(!(got_audio && got_audio_rtcp && (got_video||video_sock<0)  && (got_video_rtcp)) );
		if (!got_audio){
		...
		close_socket(audio_sock);
		close_socket(audio_rtcp_sock);
		if (video_sock>=0)
		{
			close_socket(video_sock);
			close_socket(video_rtcp_sock);
		}
	}
}
...
int linphone_core_get_local_ip_for(int type, const char *dest, char *result){
	...
	if (type==AF_INET)
		dest="8.8.8.8"; /*a public IP address*/
...
// 获取可用空闲端口
int linphone_core_get_idel_udp_port(int sport)
{
	ortp_socket_t	sock ;
...
int linphone_core_call_enable_udt_video(LinphoneCore *lc, LinphoneCall *call, bool_t enabled)
{
    if(call && (call->audiostream || call->videostream)){
		if(enabled){
			linphone_call_stop_media_streams(call);
			linphone_call_init_media_streams(call);
			call->enabled_udt = TRUE;
			linphone_call_start_media_streams(call,FALSE,FALSE);
			if(lc->vtable.display_status)
			{
				lc->vtable.display_status(lc,"Enable UDT Transport!");
			}
		}else{
			linphone_call_stop_media_streams(call);

			linphone_call_init_media_streams(call);
			call->enabled_udt = FALSE;
			linphone_call_start_media_streams(call,FALSE,FALSE);
			if(lc->vtable.display_status)
			{
				lc->vtable.display_status(lc,"Disable UDT Transport!");
			}
		}
	}
