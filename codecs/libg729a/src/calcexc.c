...
/* Local functions */
static GFLOAT gauss(INT16 *seed);
...
)
{
    ...
    GFLOAT sign[4];
    GFLOAT *cur_exc;
    GFLOAT gp, ener, fact, inter_exc, k, delta, x1, x2, g;
    ...
    if(cur_gain == (F)0.) {
        ...
        if(flag_cod != FLAG_DEC) {
            for (i_subfr = 0;  i_subfr < L_FRAME; i_subfr += L_SUBFR) {
                update_exc_err(exc_err, gp, t0);
            }
        }
        /* REMOVED -- for 6.3k mode
		else {
    ...
    for (i_subfr = 0;  i_subfr < L_FRAME; i_subfr += L_SUBFR) {
        ...
        sign[0] = (F)2. * (GFLOAT)temp2 - (F)1.;
        temp1 >>= 1;
        temp2   = (INT16)(temp1 & (INT16)0x0007);
        pos[1]  = 5 * (int)temp2 + 1;
        temp1 >>= 3;
        temp2   = (INT16)(temp1 & (INT16)0x0001);
        sign[1] = (F)2. * (GFLOAT)temp2 - (F)1.;
        temp1   = random_g729(seed);
        temp2   = (INT16)(temp1 & (INT16)0x0007);
        pos[2]  = 5 * (int)temp2 + 1;
        temp1 >>= 3;
        temp2   = (INT16)(temp1 & (INT16)0x0001);
        sign[2] = (F)2. * (GFLOAT)temp2 - (F)1.;
        temp1 >>= 1;
        ...
        temp1 >>= 4;
        temp2   = (INT16)(temp1 & (INT16)0x0001);
        sign[3] = (F)2. * (GFLOAT)temp2 - (F)1.;
        Gp      = (INT16)(random_g729(seed) & (INT16)0x1FFF); /* < 0.5  */
        gp      = (GFLOAT)Gp / (F)16384.;
        ...
        delta = (GFLOAT)sqrt(delta);
        x1 = (delta - inter_exc) * (F)0.25;
        x2 = - (delta + inter_exc) * (F)0.25;
        g = ((GFLOAT)fabs(x1) < (GFLOAT)fabs(x2)) ? x1 : x2;
        if(g >= (F)0.) {
            if(g > G_MAX) g = G_MAX;
        }
        else {
        ...
        if(flag_cod != FLAG_DEC) update_exc_err(exc_err, gp, t0);
        /* REMOVED -- for 6.3k mode
		else {
...
static GFLOAT gauss(INT16 *seed)
{
    ...
    L_acc = 0L;
    for(i=0; i<12; i++) {
        L_temp = (INT32)random_g729(seed);
        L_acc += L_temp;
    }
    L_acc >>= 7;
    temp = (INT16)L_acc;  /* Z x 512 */
    return((GFLOAT)temp * (F)0.001953125);  /* Z */
}
