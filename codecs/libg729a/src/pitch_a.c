...
static GFLOAT dot_product(GFLOAT x[], GFLOAT y[], int lg);
...
int pitch_ol_fast(         /* output: open-loop pitch lag       */
    GFLOAT signal[],        /* input : signal to compute pitch   */
                           /*         s[-PIT_MAX : l_frame-1]   */
...
)
{
    ...
    *--------------------------------------------------------------------*/
    ...
    for(i=0; i<l_frame; i+=2, p+=2)
        sum += *p * *p;
    sum = (F)1.0/(GFLOAT)sqrt(sum);          /* 1/sqrt(energy)    */
...
static GFLOAT dot_product(   /* Return the dot product between x[] an y[] */
  GFLOAT x[],                /* First vector.                             */
  GFLOAT y[],                /* Second vector.                            */
...
GFLOAT g_pitch(          /* output: pitch gain                        */
  GFLOAT xn[],           /* input : target vector                     */
  GFLOAT y1[],           /* input : filtered adaptive codebook vector */
  GFLOAT g_coeff[],      /* output: <y1,y1> and -2<xn,y1>             */
