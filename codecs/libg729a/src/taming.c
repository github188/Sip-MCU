 ...
 **************************************************************************/
int test_err(GFLOAT exc_err[4], /* (o) flag set to 1 if taming is necessary  */
...
)
{
    ...
    if(i < 0) i = 0;
    zone1 = (int) ( (GFLOAT)i * INV_L_SUBFR);

    i = t1 + L_INTER10 - 2;
    zone2 = (int)( (GFLOAT)i * INV_L_SUBFR);
...
void update_exc_err(GFLOAT exc_err[4],
	GFLOAT gain_pit,      /* (i) pitch gain */
	int t0             /* (i) integer part of pitch delay */
)
{
    int i, zone1, zone2, n;
    ...
    n = t0- L_SUBFR;
    if(n < 0) {
        temp = (GFLOAT)1. + gain_pit * exc_err[0];
        if(temp > worst) worst = temp;
        temp = (GFLOAT)1. + gain_pit * temp;
        if(temp > worst) worst = temp;
    }

    else {
        zone1 = (int) ((GFLOAT)n * INV_L_SUBFR);

        i = t0 - 1;
        zone2 = (int)((GFLOAT)i * INV_L_SUBFR);

        for(i = zone1; i <= zone2; i++) {
            temp = (GFLOAT)1. + gain_pit * exc_err[i];
            if(temp > worst) worst = temp;
        }
    }
