 ...
 */
void autocorr(
     GFLOAT *x,              /* input : input signal x[0:L_WINDOW] */
     int m,                 /* input : LPC order                  */
     GFLOAT *r               /* output: auto-correlation vector r[0:M]*/
)
{
   GFLOAT y[L_WINDOW];
   GFLOAT sum;
 ...
 *----------------------------------------------------------------------------
 */
GFLOAT levinson(         /* output: prediction error (energy) */
 GFLOAT *r,              /* input : auto correlation coefficients r[0:M] */
 GFLOAT *a,              /* output: lpc coefficients a[0] = 1 */
 GFLOAT *rc              /* output: reflection coefficients rc[0:M-1]    */
)
{
   GFLOAT s, at, err;
   int i, j, l;
...
static GFLOAT chebyshev(GFLOAT x, GFLOAT *f, int n);

void az_lsp(
  GFLOAT *a,         /* input : LP filter coefficients                     */
  GFLOAT *lsp,       /* output: Line spectral pairs (in the cosine domain) */
  GFLOAT *old_lsp    /* input : LSP vector from past frame                 */
)
{
 int i, j, nf, ip;
 GFLOAT xlow,ylow,xhigh,yhigh,xmid,ymid,xint;
 GFLOAT *pf1;

 GFLOAT f1[NC+1], f2[NC+1];
...
GFLOAT chebyshev(   /* output: the value of the polynomial C(x)       */
  GFLOAT x,         /* input : value of evaluation; x=cos(freq)       */
  GFLOAT *f,        /* input : coefficients of sum or diff polynomial */
